...
class DockerEnvironment(BaseEnvironment):
    ...
    _DOCKER_COMPOSE_AGENT_AUTH_PATH = (
        Path(__file__).parent / "docker-compose-agent-auth.yaml"
    )
    ...
    async def _run_docker_compose_command(
        ...
        compose_files = ["-f", str(self._docker_compose_path.resolve().absolute())]

        if self._use_subscription and self._DOCKER_COMPOSE_AGENT_AUTH_PATH.exists():
            compose_files.extend(
                ["-f", str(self._DOCKER_COMPOSE_AGENT_AUTH_PATH.resolve().absolute())]
            )
    ...
    async def attach(self) -> None:
        ...
        compose_file_args = ["-f", str(self._docker_compose_path.resolve().absolute())]
        if self._use_subscription and self._DOCKER_COMPOSE_AGENT_AUTH_PATH.exists():
